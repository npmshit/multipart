...
var tests = [
  { source: "nested", opts: { boundary: "AaB03x" }, chsize: 32, nparts: 2, what: "One nested multipart" },
  {
    source: "many",
...
function next() {
  ...
  dicer
    ...
    .on("finish", function() {
      ...
      else assert(error === undefined, makeMsg(v.what, "Unexpected error: " + error));
      ...
      assert.equal(
        state.parts.length,
        v.nparts,
        makeMsg(v.what, "Part count mismatch:\nActual: " + state.parts.length + "\nExpected: " + v.nparts),
      );
      ...
      assert.equal(
        partErrors,
        v.npartErrors,
        makeMsg(v.what, "Part errors mismatch:\nActual: " + partErrors + "\nExpected: " + v.npartErrors),
      );
